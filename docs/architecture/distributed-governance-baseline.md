...
Profile declaration is declarative today: a repository's declared profile selects no runtime behavior, and its only runtime consumer is the provenance stamp on sync envelopes. "Adds" in the Team Hub description states delivery staging (ADR-0019 D3: Team Hub is Stage 3), not a capability switch — the repository-local sync preparation surface is available under every declared profile, and declaring Team Hub or Organization Profile today changes provenance labeling only. The declaration mechanism, its fail-closed resolution states, and its command surface are specified in `docs/specs/deployment-profiles.md`.
