...
- a stable identifier;
- a kind: public contract, artifact, or invariant;
- a version domain;
- an owning bounded context;
- a semantic version;
- a compatibility disposition;
- the six version-domain bindings for a contract; artifact type and resolution traceability for an artifact; or resolution and ADR traceability for an invariant.

Artifact entries enumerate the eight public artifact types in the accepted Amber Core artifact model. They describe the public artifact categories required by that model and do not expose private or internal repository files.
...
The equivalent package script is `npm run contracts:validate`.
