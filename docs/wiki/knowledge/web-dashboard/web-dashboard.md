...
Document the apps/web React dashboard architecture: tRPC API, TanStack Router routes, and session/timeline components.

## Analysis Focus (from plan)
Separate package.json (@amber-protocol/web). Cover routes/, components/session/, components/timeline/, features/, lib/trpc.ts, and the Express server.
