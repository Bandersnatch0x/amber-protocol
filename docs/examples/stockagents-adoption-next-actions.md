# StockAgents Adoption Next Actions

Status: review only

Target: `D:\code_space\trae-project\StockAgents`

Source bundle: `docs/examples/stockagents-adoption-bundle/`
...
- StockAgents files written: false
- StockAgents commands executed: false
...
| --- | ---: | --- | --- |
| Missing required Harness files | 17 | StockAgents has no required Harness bootstrap files yet. | Human approval before `init` or manual adoption |
...
These are the 17 required files that explain the gate blocker. Creating them would change StockAgents and therefore requires explicit human approval.
...
- Is `python -m pytest` the correct default test command for StockAgents?
...
- Allow `node scripts/harness.js init --target "D:\code_space\trae-project\StockAgents"` to create missing Harness files without overwriting existing files.
- Request a manual patch proposal for only selected files.
- Keep StockAgents read-only and only maintain review artifacts in this repository.
...
These commands inspect or summarize from the Harness side and do not modify StockAgents:
...
node scripts/harness.js adoption report --target "D:\code_space\trae-project\StockAgents" --output-dir docs/examples/adoptions
...
Commands that write to StockAgents or execute its tests require a fresh human approval.
