# StockAgents Adoption Walkthrough

Target project: `D:\code_space\trae-project\StockAgents`
...
node scripts/harness.js audit --target "D:\code_space\trae-project\StockAgents" --summary
node scripts/harness.js adoption report --target "D:\code_space\trae-project\StockAgents" --output docs/examples/stockagents-adoption-report.md --json
node scripts/harness.js adoption report --target "D:\code_space\trae-project\StockAgents" --output-dir docs/examples --json
node scripts/harness.js init --target "D:\code_space\trae-project\StockAgents" --dry-run --json
node scripts/harness.js team inspect --target "D:\code_space\trae-project\StockAgents" --json
node scripts/harness.js team install --target "D:\code_space\trae-project\StockAgents" --version 1.0.0 --preset safe-bootstrap --json
node scripts/harness.js team update --target "D:\code_space\trae-project\StockAgents" --version 1.1.0 --dry-run --json
node scripts/harness.js doctor --target "D:\code_space\trae-project\StockAgents" --json
...
- `audit --summary` produced an 18-line bounded text report for StockAgents while preserving counts, `python -m pytest`, unknowns, and the next safe command.
- `adoption report` generated `docs/examples/stockagents-adoption-report.md` with audit, init dry-run, team distribution, and maintenance sections.
- `adoption report --output-dir` generated a non-conflicting timestamped filename and left the StockAgents root uninitialized.
...
StockAgents should not receive a full Harness init automatically. It already has substantial documentation and project-specific rules, so the safe path is:
...
This trial did not overwrite any existing StockAgents project files. The only intentional write was the V5 team metadata under `.harness/team/`.
