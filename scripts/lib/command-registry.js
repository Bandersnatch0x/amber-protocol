...
const COMMAND_HELP = {
	...
	artifact: [
		...
		"decision, gate. intent, spec, and plan each have a closed lifecycle of",
		...
		"evaluate` is that content's deterministic consumer.",
		"",
	...
	gate: [
		...
		"under the registered comparator (numeric: eq/ne/lt/le/gt/ge; string:",
		'eq/ne/contains; version, dot-numeric: eq/lt/le/gt/ge, where "1.2" <',
		'"1.10"). An expired gate (gate.expires at or before the clock,',
		"no-tolerance) refuses to run: AMBER_E_GATE_EXPIRED, no outcome",
		"appended.",
		"",
...
const COMMAND_OUTPUT = {
	...
	artifact: {
		usage: [
			"Usage: amber artifact admit --target <repo> --id <identity> --body <markdown> [--type <intent|spec|plan|decision|gate>] [--expected-head <n>] [--supersedes-revision <n>] [--idempotency-key <key>] [--transition <name>] [--trace <type>:<identity>[@<revision>]] [--scope <tag>] [--extension <ns>.<key>=<value>] [--decision-kind <acceptance|approval|review>] [--principal <id>] [--json]",
