...
const COMMAND_HELP = {
	...
	artifact: [
		...
		"        --expected-head <n> (or --supersedes-revision <n>) declares the",
		"        compare-and-swap precondition: admission fails closed as a",
		"        conflict when the head moved, so concurrent editors cannot",
		"        overwrite or duplicate a committed revision. An exact duplicate",
		"        retry (same Body, provenance, and expected head) returns the",
		"        original receipt with no new revision; the same Body with",
		"        different envelope content fails closed as an idempotency",
		"        conflict. --idempotency-key <key> is optional retry metadata —",
		"        reusing a key with different content fails closed; keys never",
		...
		'  amber artifact admit --id intent/login-bug --body "..." --expected-head 1 --target .',
...
const COMMAND_OUTPUT = {
	...
	artifact: {
		usage: [
			"Usage: amber artifact admit --target <repo> --id <identity> --body <markdown> [--expected-head <n>] [--supersedes-revision <n>] [--idempotency-key <key>] [--json]",
