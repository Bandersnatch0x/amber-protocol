...
const COMMAND_HELP = {
	...
	artifact: [
		...
		"decision. intent, spec, and plan each have a closed lifecycle of",
		...
		"Principal registry at admission time (amber principal).",
		"",
		...
		"        --type <intent|spec|plan|decision> selects the Artifact Type",
	...
	principal: [
		...
		"revoked events; no in-place mutation path), fail-closed on",
		...
		"        --issuer, and the validity window --valid-from/--valid-to",
		"        (ISO-8601; half-open [from, to)) are optional",
		"        qualifications; the window is evaluated against the",
		"        admission-time clock when a Decision binds the Principal.",
		...
		"",
		"Examples:",
		"  amber principal register --target . --id alice@example.com --kind human --role tech-lead --json",
		"  amber principal register --target . --id ci-bot --kind service --capability deploy --valid-to 2027-01-01 --json",
...
const COMMAND_OUTPUT = {
	...
	artifact: {
		usage: [
			"Usage: amber artifact admit --target <repo> --id <identity> --body <markdown> [--type <intent|spec|plan|decision>] [--expected-head <n>] [--supersedes-revision <n>] [--idempotency-key <key>] [--transition <name>] [--trace <type>:<identity>[@<revision>]] [--scope <tag>] [--extension <ns>.<key>=<value>] [--decision-kind <acceptance|approval|review>] [--principal <id>] [--json]",
	...
	principal: {
		usage: [
			"Usage: amber principal register --target <repo> --id <id> --kind <human|service> [--role <role>] [--membership <org>] [--capability <capability>] [--valid-from <iso>] [--valid-to <iso>] [--issuer <name>] [--json]",
