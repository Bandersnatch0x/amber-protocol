...
const COMMAND_HELP = {
	...
	sync: [
		...
		"  session <run|push|pull|list|replay|conflicts>",
		"        run: pull (admit + apply) then prepare transport. push: prepare",
		"        transport. Transport is PREPARATION/REPORT-ONLY (F035 D1): the",
		"        report lists envelopes, affected .amber/sync/** paths, and",
		"        proposed git operations as structured, schema-governed ops",
		"        (schemas/sync-transport-report.schema.json; verb + confined",
		"        paths — never shell strings). Amber NEVER runs git add, git",
		"        commit, or git push, and there is no --execute escape hatch.",
		"        Live transport needs its own accepted ADR (ADR-0020).",
		...
		"Options:",
		"  --target <dir>     Path to the target repository.",
		"  --execute          (drift refresh only) Actually refresh stale",
		"                     Amber-owned files (idempotent). Never transports.",
		"  --json             Emit machine-readable JSON.",
		"",
		"Examples:",
		"  amber sync --target .",
