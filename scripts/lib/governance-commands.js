...
function inspectGovernanceReadinessBody(target, options = {}) {
	...
	const outputPath = options.output ? writeReadinessMarkdown(result, options.output) : undefined;
...
function generateGovernanceReportBody(target, options = {}) {
	...
	const output =
		options.output && !path.isAbsolute(options.output)
			...
			: options.output;
	const outputPath = options.output ? writeGovernanceReportMarkdown(report, output) : undefined;
...
function mapStandardsBody(target, options = {}) {
	...
	for (const risk of result.risks) {
		const ctrl =
			risk.amberControls && risk.amberControls.length ? ` — ${risk.amberControls.join("; ")}` : "";
...
function standardsInitBody(target) {
	...
	if (!fs.existsSync(templatePath)) {
		return {
			target,
			errors: [`Security-standard template missing: ${templatePath}`],
			warnings: [],
		};
	}
