...
const MESSAGES = {
	...
	adoptionReportNoInit: "No target-repository files were initialized by this report.",
	adoptionReviewBeforeChange:
		"Review adoption gate findings before initializing or changing the target repository.",
	adoptionReadyForApproval: "Ready for human approval of the next safe Amber action.",
	...
	requiredStarterFilesPendingApproval: "## Required Amber Starter Files Pending Approval",
	...
	teamAlreadyInstalled: "Team Amber is already installed; use team update or team rollback.",
...
function formatAdoptionBoundaryLines(boundaries, keys = ADOPTION_BOUNDARY_KEYS) {
	return keys.map((key) => `- ${ADOPTION_BOUNDARY_LABELS[key]}: ${boundaries[key]}`);
}
