...
const FLAG_SPECS = {
	...
	// F054 ticket 1 (#279): Control Band detector & finding flags (--id,
	// --source, --baseline, --scope, --subject, --decision-identity, and
	// --revision are shared with existing surfaces).
	"--detector-version": { key: "detectorVersion" },
	"--metric": { key: "metric" },
	"--rule": { key: "ruleVal", accumulate: "rules" },
	"--window-ms": { key: "windowMs" },
	"--cooldown-ms": { key: "cooldownMs" },
	"--max-observations": { key: "maxObservations" },
	"--window-from": { key: "windowFrom" },
	"--window-to": { key: "windowTo" },
	"--value": { key: "value" },
	...
	// F056 ticket 1 (#288): External Effect contract flags (--id, --owner,
	// --scope, --credential, --timeout-ms, --adapter, --adapter-version,
	// --decision-identity, and --revision are shared with existing surfaces).
	"--effect-version": { key: "effectVersion" },
	"--system": { key: "system" },
