...
const CATALOG = {
	...
	AMBER_E_CONTEXT_LOADOUT_ROUTE: {
		...
		remedy: "Pass --route <id> matching a route in routes/*.json (e.g. --route feature-standard).",
		layer: "Context",
	...
	AMBER_E_CONTEXT_LOADOUT_REQUIRED_OVERFLOW: {
		...
		cause: "The pinned (required) pages' estimated words exceed the budget, so the loadout cannot be provably bounded.",
		remedy: "Raise --budget, unpin pages, or reduce the pinned pages' size. No loadout file is written on this error.",
		layer: "Context",
