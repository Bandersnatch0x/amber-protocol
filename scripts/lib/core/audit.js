...
function auditTargetRepo(targetRoot, classification) {
	...
	return {
		target: targetRoot,
		// Public read-only contract (#43): audit NEVER writes a target file. No
		// .amber/last-audit.json navigation stamp, no auditStampWritten field —
		// `amber next` advances via the audit step's advisory isDone instead.
		readOnly: true,
...
function validateHandoff(target) {
	...
	try {
		...
		const sessionsDir = statePath(targetRoot, "sessions");
