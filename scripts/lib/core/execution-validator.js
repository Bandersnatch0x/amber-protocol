...
function validateLoopContract(contractPath) {
  ...
  if (!fs.existsSync(contractPath)) {
    return { valid: false, errors: [`Contract file not found: ${contractPath}`], warnings: [], explanation: 'Contract file does not exist.' };
  }
  ...
  try {
    ...
    return { valid: false, errors: [`Invalid JSON: ${e.message}`], warnings: [], explanation: 'Contract file contains invalid JSON.' };
  }
  ...
  if (!contract.hardStops) {
    ...
    else if (typeof maxIterations !== 'number' || maxIterations <= 0) errors.push('hardStops.maxIterations must be > 0');

    if (timeout === undefined) errors.push('hardStops.timeout is required');
    else if (typeof timeout !== 'number' || timeout <= 0) errors.push('hardStops.timeout must be > 0');
...
function validateWorkflowPack(packPath) {
  ...
  if (!fs.existsSync(packPath)) {
    return { valid: false, errors: [`Pack file not found: ${packPath}`], warnings: [], unsafePatterns: [] };
  }
  ...
  try {
    ...
    return { valid: false, errors: [`Invalid JSON: ${e.message}`], warnings: [], unsafePatterns: [] };
  }
  ...
  if (/\bsh\s+|bash\s+|powershell\s+/.test(packStr)) unsafePatterns.push('shell script execution detected');
  ...
  if (pack.workflow && Array.isArray(pack.workflow)) {
    pack.workflow.forEach((step, i) => {
      if (step.script) {
        ...
        if (!fs.existsSync(scriptPath)) errors.push(`Workflow step ${i} references missing script: ${step.script}`);
      }
    });
  }
...
function validateIntegration(integrationPath) {
  ...
  if (!fs.existsSync(integrationPath)) {
    return { valid: false, sideEffects: [], credentialsRequired: false, permissionGates: [], warnings: [`Integration file not found: ${integrationPath}`] };
  }
  ...
  try {
    ...
    return { valid: false, sideEffects: [], credentialsRequired: false, permissionGates: [], warnings: [`Invalid JSON: ${e.message}`] };
  }

  const configStr = JSON.stringify(config);

  // Detect side effects
  if (/\bfile[:\.]write|writeFile|createWriteStream|fs\.write/i.test(configStr)) sideEffects.push('file_write');
  if (/\bnetwork|http[s]?:|fetch|axios|request\b/i.test(configStr)) sideEffects.push('network_call');
  if (/\bspawn|exec|child_process|shell\b/i.test(configStr)) sideEffects.push('process_spawn');
  if (/\bdb|database|sql|query|transaction\b/i.test(configStr)) sideEffects.push('database_operation');

  // Check credentials requirement
  const credentialsRequired = /\bapi[_-]?key|token|secret|password|credential|auth\b/i.test(configStr);
...
function checkExecutionReadiness(projectRoot, planPath) {
  const blockers = [];
  const warnings = [];
  const checks = { plan: false, worktree: false, policy: false, env: false, integrations: false };
  ...
  if (!fs.existsSync(planPath)) {
    ...
    try {
      ...
      const hasApproval = /<!-- gate: approved -->|<!-- approved -->/i.test(planContent) ||
  ...
  if (fs.existsSync(gitDir)) {
    try {
      ...
      if (fs.existsSync(mergeHead) || fs.existsSync(rebaseApply) || fs.existsSync(rebaseMerge)) {
        ...
        // Check git index for recent modifications (heuristic: modified in last 5 minutes)
        const indexPath = path.join(gitDir, 'index');
        if (fs.existsSync(indexPath)) {
          const indexStat = fs.statSync(indexPath);
          const ageMs = Date.now() - indexStat.mtimeMs;
          if (ageMs < 300000) { // 5 minutes
            warnings.push('Worktree may have recent uncommitted changes (index recently modified)');
          } else {
...
module.exports = { validateLoopContract, validateWorkflowPack, validateIntegration, checkExecutionReadiness };
