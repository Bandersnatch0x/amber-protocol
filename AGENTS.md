...
- `node scripts/amber.js adapter <register|read|show|list|receipts> --target <repo>` - register and read through pre-Cutover read-only Adapters (F051): each Adapter declares source owner, record types/versions, exact scope, identity mapping, freshness, and read-only permissions. Adapter reads preserve source bytes/digest, identity, scope, provenance, and adapter version in append-only read receipts under `.amber/adapters/`; they may write only Adapter governance ledgers and never mutate Canonical Artifacts, so the external source remains authoritative until an explicit Cutover Decision.
