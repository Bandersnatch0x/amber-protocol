...
- `node scripts/amber.js adapter <register|read|show|list|receipts> --target <repo>` - register and read through pre-Cutover read-only Adapters (F051): each Adapter declares source owner, record types/versions, exact scope, identity mapping, freshness, and read-only permissions. Adapter reads preserve source bytes (base64), raw-byte digest, byte length, record identity/type/version, scope, provenance, adapter version, and explicit status (`fresh|stale|unavailable|conflict|unmapped`) in append-only read receipts under `.amber/adapters/`; they may write only Adapter governance ledgers and never mutate Canonical Artifacts, so the external source remains authoritative until an explicit Cutover Decision.
- `node scripts/amber.js maintain <register-detector|detect|propose|triage|complete|rollup|detectors|findings|proposals> --target <repo>` - Control Band detectors and deterministic Findings (F054): a detector is a versioned, model-independent definition (metric, source, numeric baseline, closed comparator rules, window, scope, cooldown, observation ceiling, declared service owner, optional Policy pin) registered behind a single-use committed human Decision into the hash-chained ledgers under `.amber/maintain/`. `detect` is target-read-only — only an out-of-band verdict appends one immutable, fingerprint-correlated Finding; `propose` derives one open Trigger Proposal per detector + subject from recorded Findings only (never caller input); `triage` binds it to the declared service owner's single-use human Decision (`fix|schedule|dismiss` — only `fix` returns a CANDIDATE Intent payload that still passes normal canonical admission, so nothing is auto-admitted); `complete` pins the shipped fix to committed intent/eval/eval-result revisions; staleness derives at read time, never edited in place.
- `node scripts/amber.js retention <classify|evaluate|classifications|hold|release|holds|holder|holders|candidate|authorize|candidates|execute|settle|status|proof> --target <repo>` - governed retention classes and deterministic expiry (F055): classifications bind committed records to protocol classes (`ephemeral|operational|governance|audit`) whose TTL and legal basis resolve from a committed tenant retention Policy, appended immutably to the hash-chained ledgers under `.amber/retention/`; `evaluate` derives `expired-eligible|retained` read-only from the latest classification plus the injected clock. Legal Holds (create and release each a single-use human Decision; released holds stay listable forever) outrank TTL; deletion candidates enumerate exact records and registered Holders, `authorize` consumes one drift-bound single-use Approval, every Holder settles independently (`deletion-pending` until full coverage), and `proof` derives the minimal Deletion Proof — Amber deletes no bytes, and deleted records project as tombstones that refuse Gates.
- `node scripts/amber.js external <register|effects|propose|authorize|proposals|execute|settle|reconcile|status|compensate|transactions> --target <repo>` - registered External Effect contracts (F056): the only thing that can ever execute externally is a registered contract (owner, closed system type, operation name, exact target/scope, declared input schema, idempotency, credentials class, required receipt fields, a named compensating effect or explicit `--irreversible`, bounded timeout, verified F051 Adapter pin), registered behind a single-use committed human Decision into the hash-chained ledgers under `.amber/external/`. `propose` requires the explicit `--yes`, `authorize` consumes one drift-bound single-use Approval, missing output never reads as success (`unknown` reaches `committed` only via `reconcile` with independent Evidence), compensation is a new governed effect, and MCP surfaces external writes as exactly one approval-required submission (`amber.external.propose`) it never spawns.
