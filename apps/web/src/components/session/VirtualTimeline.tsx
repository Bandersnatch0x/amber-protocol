...
export function VirtualTimeline({ events, onEventClick, autoScroll = true }: VirtualTimelineProps) {
  ...
  useEffect(() => {
    if (autoScroll && events.length > 0 && parentRef.current) {
      const isNearBottom =
        parentRef.current.scrollHeight - parentRef.current.scrollTop - parentRef.current.clientHeight < 200;
  ...
  return (
    <div className="relative">
      <div ref={parentRef} className="h-[600px] overflow-auto rounded border border-slate-200 dark:border-slate-700">
        <div
          style={{
      ...
      {showScrollControls && events.length > 10 && (
        <div className="pointer-events-none absolute bottom-3 right-3 flex items-center gap-1.5 z-10">
          <button
            ...
            className="pointer-events-auto inline-flex items-center gap-1 rounded-md border border-slate-200 bg-white/95 px-2.5 py-1 text-xs font-medium text-slate-700 shadow-sm backdrop-blur hover:bg-slate-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 dark:border-slate-700 dark:bg-slate-800/95 dark:text-slate-200 dark:hover:bg-slate-700"
          >
