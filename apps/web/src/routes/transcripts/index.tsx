...
function TranscriptsPage() {
  ...
  const source = useMemo(() => firstAvailableSource(transcripts ?? []), [transcripts]);
  ...
  return (
    <div className="page-container space-y-6">
      <header className="space-y-2">
        ...
        {isLoading ? (
          // Never flash a "0 transcripts" count while the list is loading.
          <div
          ...
          !error && (
            <p className="text-sm text-slate-600 dark:text-slate-400">
              {t(filtered.length === 1 ? 'transcripts.countOne' : 'transcripts.count', {
        ...
        <section className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 dark:border-slate-700 dark:bg-slate-800 lg:grid-cols-2">
          <div className="min-w-0">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
              {t('transcripts.repositoryDirectory')}
            </p>
            <p className="mt-1 break-all font-mono text-sm text-slate-800 dark:text-slate-100">
              {source.repoPath ?? t('transcripts.unknown')}
            </p>
          </div>
          <div className="min-w-0">
            <p className="text-xs font-medium uppercase tracking-wide text-slate-500 dark:text-slate-400">
              {t('transcripts.transcriptDirectory')}
            </p>
            <p className="mt-1 break-all font-mono text-sm text-slate-800 dark:text-slate-100">
              {source.sourceDirectory ?? t('transcripts.unknown')}
            </p>
          </div>
        </section>
      )}
              ...
              <div className="flex items-start justify-between gap-4">
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-slate-500 dark:text-slate-400">
                    <span>
                      {t(
                        transcript.turnCount === 1 ? 'transcripts.turnsOne' : 'transcripts.turns',
                        { count: transcript.turnCount },
                      )}
                    </span>
                    <span>
                ...
                <svg
                  aria-hidden="true"
                  className="mt-1 h-4 w-4 flex-shrink-0 text-slate-300 dark:text-slate-600"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                  strokeWidth={2}
