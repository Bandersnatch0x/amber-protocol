...
function RoutesPage() {
  ...
  const filteredGrouped = grouped && typeof grouped === 'object'
    ? Object.fromEntries(
    ...
    <div className="page-container">
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full max-w-md px-3 py-2 text-sm border border-slate-300 dark:border-slate-600 rounded-md bg-white dark:bg-slate-800 text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        />
      </div>
      ...
      {totalRoutes === 0 && !isLoading && (
        <div className="card p-12 text-center">
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {searchQuery ? 'Try a different search term' : 'No route definitions available'}
          </p>
        </div>
      )}
