...
Coding Harness 是一个仓库本地的 Agent 辅助工程操作套件。它用于安装、审计、验证和维护一组小而明确的项目文件，帮助 Agent 理解项目、显式记录功能状态，并在会话之间可靠交接。
...
![Coding Harness 安全 bootstrap 信息图](./assets/readme/coding-harness-cover.png)

## 架构

```mermaid
flowchart LR
  CLI["scripts/harness.js<br/>统一 CLI"] --> Core["scripts/lib/harness-core.js<br/>确定性操作"]
  Core --> Templates["templates/<br/>Harness 初始文件"]
...
- `scripts/harness.js` 负责命令路由和面向用户的输出。
- `scripts/lib/harness-core.js` 包含确定性的 scaffold、audit、adoption、planning、review、team 和 maintenance 逻辑。
...
运行 `node scripts/harness.js <command> --help` 查看具体命令帮助。

## 会安装什么

`doctor` 检查的最小 Harness 文件：
...
- `adoption selected-files` 只接受安全的相对已知 Harness 文件路径，并且只写入指定的提案文件。
...
| 阶段 | 状态 | 范围 |
| --- | --- | --- |
| V1 Safe Harness Bootstrap | 已实现 | `init`、`audit`、`wiki`、`doctor`、`handoff` |
