...
test("audit ignores dependency and generated-output markdown noise", () => {
	...
	fs.writeFileSync(
		path.join(target, "env", "Lib", "site-packages", "demo", "README.md"),
		"# Dependency docs\n",
	);
	fs.writeFileSync(
		path.join(target, "env_new", "Lib", "site-packages", "demo", "README.md"),
		"# Dependency docs\n",
	);
	fs.writeFileSync(
		path.join(target, ".venv", "Lib", "site-packages", "demo", "README.md"),
		"# Dependency docs\n",
	);
	fs.writeFileSync(
		path.join(target, "results", "600000", "reports", "market_report.md"),
		"# Generated result\n",
	);
...
test("audit reports tooling evidence without inventing commands", () => {
	...
	assert.ok(
		result.toolingEvidence.some(
			(item) => item.source === "package-lock.json" && item.name === "npm",
		),
	);
	assert.ok(
		result.toolingEvidence.some((item) => item.source === "pnpm-lock.yaml" && item.name === "pnpm"),
	);
	assert.ok(
		result.toolingEvidence.some(
			(item) => item.source === "pyproject.toml" && item.name === "python",
		),
	);
	assert.ok(
		result.unknowns.some((unknown) =>
			/candidate commands proposed below require project-owner confirmation/.test(unknown),
		),
	);
});
...
test("audit suggests Python candidate verification commands without confirming them", () => {
	...
	assert.ok(
		result.unknowns.some((unknown) =>
			/candidate verification commands require confirmation/i.test(unknown),
		),
	);
});
...
test("doctor aggregates V1 Harness guardrails for a broken Harness", () => {
	...
	assert.ok(
		result.errors.some((error) => /AGENTS\.md does not route agents to docs\/wiki/.test(error)),
	);
	assert.ok(
		result.errors.some((error) =>
			/verification\.md does not contain a verification command block/.test(error),
		),
	);
	assert.ok(
		result.errors.some((error) => /PROGRESS\.md does not contain a next action/.test(error)),
	);
	assert.ok(
		result.errors.some((error) =>
			/session-handoff\.md must include a non-empty Summary section/.test(error),
		),
	);
});
