...
test("rules init writes .amber/governance/rules.json from defaults (idempotent)", () => {
	const dir = tmpDir();
	const r1 = dispatch("governance", { _: ["rules", "init"], target: dir });
	...
	const r2 = dispatch("governance", { _: ["rules", "init"], target: dir });
...
test("rules inspect reports the active policy surface", () => {
	const dir = tmpDir();
	dispatch("governance", { _: ["rules", "init"], target: dir });
	const r = dispatch("governance", { _: ["rules", "inspect"], target: dir });
...
test("rules check requires --command", () => {
	const dir = tmpDir();
	const r = dispatch("governance", { _: ["rules", "check"], target: dir });
