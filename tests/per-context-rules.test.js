...
test("a global deny is NOT overridden by a context allow (deny-wins is absolute)", () => {
  ...
  fs.writeFileSync(path.join(dir, ".amber", "governance", "rules.json"),
    JSON.stringify({ schemaVersion: 1, defaultAction: "deny", rules: [
      { id: "global-deny-rm", action: "deny", match: "regex", pattern: "rm\\s+-rf" }] }));
  ...
  const pack = {
    id: "p", version: "1", loopContracts: [{
      ...
      stateSpine: ".amber/loops/c1/state.json", hardStops: { maxIterations: 1 },
      reviewGates: ["human-approval"],
      governed: { command: "rm -rf /tmp/x", requiresApproval: true, rules: [
        { id: "ctx-allow-rm", action: "allow", match: "regex", pattern: "rm\\s+-rf" }] },
      execution: { executesAnything: false } }],
  };
  const packPath = path.join(dir, "pack.json");
...
test("a context deny blocks a command even if global rules would allow it", () => {
  ...
  fs.writeFileSync(path.join(dir, ".amber", "governance", "rules.json"),
    JSON.stringify({ schemaVersion: 1, defaultAction: "deny", rules: [
      { id: "global-allow-node", action: "allow", match: "prefix", pattern: "node " }] }));
  ...
  const pack = {
    id: "p", version: "1", loopContracts: [{
      ...
      stateSpine: ".amber/loops/c1/state.json", hardStops: { maxIterations: 1 },
      reviewGates: ["human-approval"],
      governed: { command: "node -e 0", requiresApproval: true, rules: [
        { id: "ctx-deny-node-e", action: "deny", match: "regex", pattern: "node\\s+-e" }] },
      execution: { executesAnything: false } }],
  };
  const packPath = path.join(dir, "pack.json");
  fs.writeFileSync(packPath, JSON.stringify(pack));
