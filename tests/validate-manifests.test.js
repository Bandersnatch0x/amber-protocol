...
function runValidateManifests(args) {
  return spawnSync(process.execPath, [path.join(ROOT, "scripts", "validate-manifests.js"), ...args], {
    cwd: ROOT,
    encoding: "utf8"
  });
}
...
test("manifest validator reports missing required manifest", () => {
  ...
  assert.ok(result.errors.some((error) => error.includes("Missing required manifest: .codex-plugin/plugin.json")));
});
...
test("validate-manifests wrapper returns parseable JSON and non-zero on failure", () => {
  const result = runValidateManifests(["--target", fixture("bad-manifests-missing-skills"), "--json"]);
