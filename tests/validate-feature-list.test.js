...
} = require("../scripts/lib/core/validators");
...
test("template feature_list.json is valid", () => {
	const result = validateFeatureListFile(
		path.join(__dirname, "..", "templates", "feature_list.json"),
	);
...
test("feature list schema rejects malformed root and features array", () => {
	assert.ok(
		validateFeatureListData(null).errors.some((error) => /must contain an object/.test(error)),
	);
	assert.ok(
		validateFeatureListData([]).errors.some((error) => /must contain an object/.test(error)),
	);
...
test("feature list schema rejects duplicate ids and invalid status", () => {
	const result = validateFeatureListData({
		features: [validFeature({ id: "F001" }), validFeature({ id: "F001", status: "unknown" })],
	});
