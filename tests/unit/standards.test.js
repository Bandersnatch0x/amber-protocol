...
test("maps all 10 ASI risks with honest, valid coverage labels", () => {
	...
	assert.ok(
		r.risks.every((x) => ["governance", "partial", "out-of-scope"].includes(x.amberCoverage)),
	);
	...
	assert.ok(
		r.disclaimer.toLowerCase().includes("not a certification") ||
			r.disclaimer.toLowerCase().includes("self-assessment"),
	);
...
test("a rules.json with a deny rule marks ASI02 (not ASI04) present — control-specific", () => {
	...
	fs.writeFileSync(
		...
		JSON.stringify({
			rules: [{ id: "x", action: "deny", match: "regex", pattern: "rm", mapsTo: ["ASI04"] }],
		}),
	);
	const r = mapStandards(dir, "owasp-agentic");
...
test("a corrupt framework file reports corruption, not 'unknown framework'", () => {
	...
	assert.ok(
		!/unknown framework/i.test(msg),
		`should not say 'Unknown framework' for a corrupt file, got: ${msg}`,
	);
	fs.rmSync(dir, { recursive: true, force: true });
});
