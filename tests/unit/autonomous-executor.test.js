...
describe("autonomous-executor", () => {
  ...
  beforeEach(() => {
    ...
    const sessionsDir = path.join(testRoot, ".harness", "sessions", "test-session-id");
    ...
    const policy = {
      ...
      retry: { maxAttempts: 3, backoffMs: [100, 200, 500], retryableStages: ["implement", "verify"] },
      budget: { onExceed: "pause" },
    };
    fs.writeFileSync(path.join(harnessDir, "autonomous-policy.json"), JSON.stringify(policy, null, 2));
  ...
  it("should return error for non-existent session", async () => {
    const result = await executeAutonomous(testRoot, "nonexistent-id", { dryRun: true });
