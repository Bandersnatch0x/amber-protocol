...
test("rules.json with a deny rule marks ASI02 present", () => {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ schemaVersion: 1, defaultAction: "deny", rules: [
      { id: "d", action: "deny", match: "regex", pattern: "rm -rf", mapsTo: ["ASI02"] }] }),
  );
  const r = mapStandards(dir, "owasp-agentic");
...
test("rules.json with an allow rule marks ASI04 present (command pinning)", () => {
  ...
  fs.writeFileSync(
    path.join(dir, ".amber", "governance", "rules.json"),
    JSON.stringify({ schemaVersion: 1, defaultAction: "deny", rules: [
      { id: "a", action: "allow", match: "prefix", pattern: "node ", mapsTo: ["ASI04"] }] }),
  );
  const r = mapStandards(dir, "owasp-agentic");
...
test("runtime-only risks (ASI01/03/05/07/08/10) are never present regardless of state", () => {
  ...
  fs.writeFileSync(
    path.join(dir, ".amber", "governance", "rules.json"),
    JSON.stringify({ rules: [{ id: "x", action: "deny", mapsTo: ["ASI01", "ASI03", "ASI05", "ASI07", "ASI08", "ASI10"] }] }),
  );
  const r = mapStandards(dir, "owasp-agentic");
  for (const id of ["ASI01", "ASI03", "ASI05", "ASI07", "ASI08", "ASI10"]) {
    assert.equal(r.risks.find((x) => x.id === id).present, false, `${id} must stay out-of-scope/present=false`);
  }
  fs.rmSync(dir, { recursive: true, force: true });
});
