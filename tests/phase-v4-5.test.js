...
function tempDir(name) {
	return fs.mkdtempSync(path.join(os.tmpdir(), `amber-v4-5-${name}-`));
}
...
function preparedTarget(name) {
	...
	assert.equal(
		runHarness(["plan", "--target", target, "--feature", "F001", "--title", "Agent slice"]).status,
		0,
	);
	...
	assert.equal(
		runHarness(["task", "prepare", "--target", target, "--plan", plan, "--task", "slice-1"]).status,
		0,
	);
	return target;
}
