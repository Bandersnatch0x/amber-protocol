...
test("team inspect exposes registry metadata and compatibility matrix", () => {
	...
	assert.deepEqual(
		payload.registry.presets.map((preset) => preset.id),
		["safe-bootstrap"],
	);
	assert.deepEqual(
		payload.registry.rulePacks.map((pack) => pack.id),
		["amber-delivery"],
	);
...
test("team install update rollback and pin preserve target customizations", () => {
	...
	assert.match(
		JSON.parse(blockedUpdate.stdout).errors.join("\n"),
