...
test("adoption report aggregates safe trial steps without initializing target", () => {
	...
	assert.equal(
		payload.sections.map((section) => section.id).join(","),
		"audit,init-dry-run,team,maintenance",
	);
...
test("adoption report output-dir creates non-conflicting timestamped reports", () => {
	...
	assert.match(
		path.basename(firstPayload.reportPath),
		/adoption-output-dir-target-.*-adoption-report-\d{4}-\d{2}-\d{2}t\d{2}-\d{2}-\d{2}/i,
	);
...
test("adoption compare auto-selects the latest two reports and reports deltas", () => {
	...
	fs.writeFileSync(
		...
		[
			...
			"## Audit Summary",
			"",
			"- Existing Amber starter files: 10",
			"- Missing Amber starter files: 5",
			"- Existing docs: 2",
			"- Wiki-like files: 1",
			"- Conflicts: 0",
			"",
	...
	fs.writeFileSync(
		...
		[
			...
			"## Audit Summary",
			"",
			"- Existing Amber starter files: 12",
			"- Missing Amber starter files: 3",
			"- Existing docs: 4",
			"- Wiki-like files: 2",
			"- Conflicts: 0",
			"",
...
test("adoption compare writes markdown diff and refuses to overwrite it", () => {
	...
	assert.match(diff, /Missing Amber starter files/);
...
test("adoption bundle writes a review bundle with manifest and refuses overwrite", () => {
	...
	assert.deepEqual(payload.files.map((file) => file.relativePath).sort(), [
		"README.md",
		"diff.md",
		"gate.md",
		"index.md",
...
test("adoption next-actions writes a gate checklist from a bundle and refuses overwrite", () => {
	...
	assert.deepEqual(
		payload.approvalGates.map((gate) => gate.id),
...
test("adoption apply-plan requires dry-run in V1", () => {
	...
	fs.writeFileSync(
		path.join(bundleDir, "manifest.json"),
		`${JSON.stringify({ kind: "adoption-bundle", target }, null, 2)}\n`,
	);
...
test("adoption selected-files writes a proposal with included files and refuses overwrite", () => {
	...
	assert.equal(payload.target, target);
	assert.equal(payload.outputPath, output);
	assert.deepEqual(payload.selectedFiles, [
		"AGENTS.md",
		"docs/wiki/index.md",
		"docs/wiki/product/feature-map.md",
	]);
